...
layui.define(['jquery', 'layer'], function(exports){
	...
	var obj = function(config){
		...
		this.config = {
			...
			tips: getLanguage('SelectM.s1'), //'请选择 最多 {max} 个',				
		...
		this.getData = function(url){
			...
			$.ajax({
				...
				error: function(){
					console.error(MOD_NAME + ' hint： ' + getLanguage('SelectM.s2'));
					d = false;
				}
			});
			return d;
  ...
  obj.prototype.render = function(){
		...
		if(Object.prototype.toString.call(c.data)!='[object Array]'){
			...
			if(data===false){
				console.error(MOD_NAME + ' hint：' + getLanguage('SelectM.s3'));
				return false;
			}
			o.config.data =  data;
		}
		...
		$E.on('click','dd',function(e){
			...
			//选中
				...
				if(o.selected.length >= c.max){
					...
					layer.tips(getLanguage('SelectM.s5') +c.max, c.elem+' .multiple', {
						tips: 3,
						time: 1000,
						end:function(){					
