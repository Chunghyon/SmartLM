...
app.MapPost("/Device/Keepalive", (KeepaliveRequest request, HttpContext httpContext, StateStore store) =>
{
    ...
    LogHub.Instance.Info($"[Keepalive] ¼ö½Å: ´Ü¸»±â {request.SN} (IP: {deviceIp})");

    var response = store.UpsertKeepalive(request, deviceIp);

    if (response.AddPeople.HasValue && response.AddPeople.Value > 0)
    {
        LogHub.Instance.Info($"[Keepalive] ÀÀ´ä: ´Ü¸»±â {request.SN}¿¡ AddPeople={response.AddPeople} Àü¼Û -> »ç¿ëÀÚ ´Ù¿î·Îµå ´ë±â Áß");
    }
...
app.MapPost("/admin/devices/{sn}/request-add-people", (string sn, StateStore store) =>
{
    ...
    LogHub.Instance.Info($"[Å¬¶óÀÌ¾ðÆ® ¿äÃ»] ´Ü¸»±â {sn}¿¡ »ç¿ëÀÚ Àü¼Û ¿äÃ» -> ´ë±â ÁßÀÎ »ç¿ëÀÚ: {count}¸í");
...
app.MapPost("/admin/people/fix-timegroup", (StateStore store) =>
{
    ...
    LogHub.Instance.Info($"[°ü¸®ÀÚ] {count}¸íÀÇ »ç¿ëÀÚ TimegroupÀ» 1·Î ¼öÁ¤ ¿Ï·á");
...
app.MapPost("/admin/devices/{sn}/reset-pending", (string sn, StateStore store) =>
{
    ...
    LogHub.Instance.Info($"[°ü¸®ÀÚ] ´Ü¸»±â {sn}ÀÇ pending »óÅÂ ÃÊ±âÈ­ ¿Ï·á");
...
app.MapPost("/People/DownloadPeopleListResult", (DownloadPeopleListResultRequest request, StateStore store) =>
{
    ...
    {
        LogHub.Instance.Warn($"[°á°ú] ´Ù¿î·Îµå ½ÇÆÐ: ´Ü¸»±â {request.SN} - ¼º°ø {request.SuccessCount}¸í, ½ÇÆÐ {request.FailCount}¸í");

        if (request.FailList != null && request.FailList.Count > 0)
        {
            foreach (var fail in request.FailList.Take(3))
            {
                LogHub.Instance.Warn($"  ½ÇÆÐ: UserID={fail.UserID}, ¿¡·¯={fail.ErrMsg}");
            }
        }
    }
    else if (request.SuccessCount > 0)
    {
        LogHub.Instance.Info($"[°á°ú] ´Ù¿î·Îµå ¼º°ø: ´Ü¸»±â {request.SN}¿¡ {request.SuccessCount}¸í Á¤»ó ÀúÀå ¿Ï·á!");
    }
...
app.MapPost("/People/PushPeople", async (HttpRequest httpRequest, StateStore store) =>
{
    ...
    if (httpRequest.HasFormContentType)
    {
        ...
        if (!string.IsNullOrWhiteSpace(detailJson))
        {
            ...
            LogHub.Instance.Info($"[PushPeople] Detail JSON ¹ÞÀ½: {detailJson}");
        ...
        if ((people == null || people.Count == 0) && !string.IsNullOrWhiteSpace(form["UserID"].ToString()))
        {
            ...
            LogHub.Instance.Info($"[PushPeople] Detail ¾øÀ½, UserID ÇÊµå »ç¿ë: {userId}");
        }
    }
    else
    {
        ...
        if ((people == null || people.Count == 0) && payload?["UserID"] != null)
        {
            ...
            {
                people = new List<PersonInfo> { new PersonInfo { UserID = userId } };
                LogHub.Instance.Info($"[PushPeople] PeopleList ¾øÀ½, UserID ÇÊµå »ç¿ë: {userId}");
            }
        }
    }
    ...
    {
        LogHub.Instance.Info($"[PushPeople] ´Ü¸»±â {sn}¿¡¼­ {people.Count}¸í ¾÷·Îµå (PushType={pushType} - {operation})");
        foreach (var person in people.Take(3))
        {
            ...
            LogHub.Instance.Info($"  [ºñ±³¿ë] Password={person.Password ?? "null"}, " +
    ...
    switch (pushType)
    {
        ...
        case 3: // Delete
            ...
            LogHub.Instance.Info($"[PushPeople-Delete] ´Ü¸»±â {sn}¿¡¼­ {success}¸í »èÁ¦ Ã³¸® ¿Ï·á");
            break;

        case 4: // Query - just return success, no state change
            success = people?.Count ?? 0;
            LogHub.Instance.Info($"[PushPeople-Query] ´Ü¸»±â {sn}¿¡¼­ {success}¸í Á¶È¸");
            break;
...
static IResult DownloadPeopleList(DownloadPeopleListRequest request, StateStore store)
{
    ...
    LogHub.Instance.Info($"[´Ù¿î·Îµå] ¿äÃ»: ´Ü¸»±â {request.SN}°¡ »ç¿ëÀÚ ¸ñ·Ï ¿äÃ»");
    LogHub.Instance.Info($"[´Ù¿î·Îµå] ÀÀ´ä: ´Ü¸»±â {request.SN}¿¡ {people.Count}¸íÀÇ »ç¿ëÀÚ Á¤º¸ Àü¼Û");
    ...
    if (people.Count > 0)
    {
        ...
        LogHub.Instance.Info($"  »ùÇÃ: UserID={firstPerson.UserID}, ÀÌ¸§={firstPerson.Name}, »çÁø={(hasPhoto ? $"{photoSize}bytes" : "¾øÀ½")}");

        // Log all fields of first person for debugging
        LogHub.Instance.Info($"  »ó¼¼: Password={firstPerson.Password ?? "(null)"}, CardNum={firstPerson.CardNum ?? "(null)"}, " +
        ...
        LogHub.Instance.Info($"  Åë°è: »çÁø ÀÖÀ½={withPhoto}¸í, ÃÑ »çÁø Å©±â={totalPhotoSize}bytes ({totalPhotoSize/1024}KB)");
    }
    ...
    try
    {
        ...
        if (people.Count > 0)
        {
            ...
            LogHub.Instance.Info($"[µð¹ö±×] ÀüÃ¼ JSONÀ» ÆÄÀÏ·Î ÀúÀå: {debugPath}");
        }

        var preview = json.Length > 800 ? json.Substring(0, 800) + "..." : json;
        LogHub.Instance.Info($"[ÀÀ´ä] JSON (Ã³À½ 800ÀÚ): {preview}");
    }
    catch (Exception ex)
    {
        LogHub.Instance.Error($"JSON Á÷·ÄÈ­ ½ÇÆÐ: {ex.Message}");
    }
