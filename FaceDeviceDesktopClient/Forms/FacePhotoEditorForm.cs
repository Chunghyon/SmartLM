...
public partial class FacePhotoEditorForm : Form
{
    ...
    private void BtnOK_Click(object? sender, EventArgs e)
    {
        try
        {
            ...
            {
                finalImage.Dispose();
                MessageBox.Show(reason, "얼굴 인식 검사",
    ...
    private static bool TryValidateSingleFace(Bitmap image, out string reason)
    {
        ...
        try
        {
            ...
            if (faces.Length == 0)
            {
                reason = "얼굴을 찾지 못했습니다.\n가이드 원 안에 얼굴이 크게 오도록 조정한 뒤 다시 시도하세요.";
                return false;
            }

            if (faces.Length > 1)
            {
                reason = $"얼굴이 {faces.Length}개 감지되었습니다.\n한 사람만 나오도록 사진을 조정하세요.";
                return false;
            }
