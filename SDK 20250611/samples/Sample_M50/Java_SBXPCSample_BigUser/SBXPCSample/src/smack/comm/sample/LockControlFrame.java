...
public class LockControlFrame extends JFrame {
	...
	private JTextField textDoorOpenTime;
	...
	public LockControlFrame() {
		...
		panel.setBorder(new TitledBorder(UIManager.getBorder("TitledBorder.border"), "Unconditional Open With Time", TitledBorder.LEADING, TitledBorder.TOP, null, new Color(0, 0, 0)));
		panel.setBounds(10, 211, 402, 72);
		getContentPane().add(panel);
		
		JLabel lblOpenTimeseconds = new JLabel("Open time(seconds):");
		lblOpenTimeseconds.setBounds(48, 26, 119, 29);
		panel.add(lblOpenTimeseconds);
		
		textDoorOpenTime = new JTextField();
		textDoorOpenTime.setText("5");
		textDoorOpenTime.setBounds(170, 26, 60, 29);
		panel.add(textDoorOpenTime);
		
		JButton btnOpenExt = new JButton("Open");
		btnOpenExt.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				btnOpenExt_actionPerformed(arg0);
			}
		});
		btnOpenExt.setBounds(299, 26, 93, 29);
		panel.add(btnOpenExt);
	 ...
	 private void btnGetDoorStatus_actionPerformed(ActionEvent evt) {//GEN-FIRST:event_btnGetDoorStatus_actionPerformed
	    ...
	    private void btnOpenExt_actionPerformed(ActionEvent evt) {
	    	int time = Integer.parseInt(textDoorOpenTime.getText());
	    	if (time <= 0)
	    	{
			...
			output = SysUtil.MakeXMLCommandHeader("OpenDoor");
			output = SBXPCProxy.XML_AddLong(output.value, "OpenTime", time);

			output = SBXPCProxy.GeneralOperationXML(SysUtil.MachineNumber, output.value);

			if (output.isSuccess()) {
				lblMessage.setText("Door open OK.");
			} else {
				lblMessage.setText("Door open failed.");
			}
		}
}
