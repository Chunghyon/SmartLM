...
namespace SBXPCDLLSampleCSharp
{
	public partial class frmLog : Form
    {
		...
		private void prepareGLog()
		{
            ...
            grdGlog.Columns["colBodyTemperature"].Visible = chkReadGlogExt.Checked;
            grdGlog.Columns["colMood"].Visible = chkReadGlogExt.Checked;
            grdGlog.Columns["colAttendOnly"].Visible = chkReadGlogExt.Checked;
            grdGlog.Columns["colExpired"].Visible = chkReadGlogExt.Checked;
            grdGlog.Columns["colLatitude"].Visible = chkReadGlogGps.Checked;
            grdGlog.Columns["colLongitude"].Visible = chkReadGlogGps.Checked;
            prevGlogIndex = -1;
        }
        ...
        private void cmdGlogData_Click(object sender, EventArgs e)
        {
            ...
            if (chkReadGlogGps.Checked)
            {
                if (chkReadGlogExt.Checked)
                    vRet = sbxpc.SBXPCDLL.ReadGeneralLogData_GPS_Ext(Program.gMachineNumber, chkReadMark.Checked ? (byte)1 : (byte)0);
                else
                    vRet = sbxpc.SBXPCDLL.ReadGeneralLogData_GPS(Program.gMachineNumber, chkReadMark.Checked ? (byte)1 : (byte)0);
            }
            else
            {
                if (chkReadGlogExt.Checked)
            ...
            if (vRet)
            {
				...
				while (true)
				{
					GLogInfo gi = new GLogInfo(chkReadAsGlogMark.Checked);
                    if (chkReadGlogGps.Checked)
                    {
                        if (chkReadGlogExt.Checked)
                            vRet = sbxpc.SBXPCDLL.GetGeneralLogData_GPS_Ext(Program.gMachineNumber,
                                                        out gi.tmno,
                                                        out gi.seno,
                                                        out gi.smno,
                                                        out gi.vmode,
                            ...
                            vRet = sbxpc.SBXPCDLL.GetGeneralLogData_GPS(Program.gMachineNumber,
                                                        out gi.tmno,
                                                        out gi.seno,
                                                        out gi.smno,
                                                        out gi.vmode,
        ...
        private void cmdAllGLogData_Click(object sender, EventArgs e)
        {
            ...
            {
                if (chkReadGlogExt.Checked)
                    vRet = sbxpc.SBXPCDLL.ReadAllGLogData_GPS_Ext(Program.gMachineNumber);
                else
                    vRet = sbxpc.SBXPCDLL.ReadAllGLogData_GPS(Program.gMachineNumber);
            }
            else
            {
                if (chkReadGlogExt.Checked)
            ...
            if (vRet)
            {
				...
				while (true)
				{
                    ...
                    {
                        if (chkReadGlogExt.Checked)
                            vRet = sbxpc.SBXPCDLL.GetAllGLogData_GPS_Ext(Program.gMachineNumber,
                                                    out gi.tmno,
                                                    out gi.seno,
                                                    out gi.smno,
                                                    out gi.vmode,
                            ...
                            vRet = sbxpc.SBXPCDLL.GetAllGLogData_GPS(Program.gMachineNumber,
                                                    out gi.tmno,
                                                    out gi.seno,
                                                    out gi.smno,
                                                    out gi.vmode,
		...
		private void cmdGetSLogPosInfo_Click(object sender, EventArgs e)
		{
			...
			if (sbxpc.SBXPCDLL.GeneralOperationXML(Program.gMachineNumber, ref strXML))
			{
				...
				lblMessage.Text = "LogCount=" + log_count.ToString() + 
					...
					", ValidPos: " + start_pos.ToString() + " ~ " + ((start_pos + log_count) % (max_count + 1)).ToString();
			}
			else
			{
				int vErrorCode;
		...
		private void cmdReadGLogWithPos_Click(object sender, EventArgs e)
		{
            ...
            {
                if (chkReadGlogExt.Checked)
                    ret = sbxpc.SBXPCDLL.ReadGLogWithPos_GPS_Ext(Program.gMachineNumber, start, end);
                else
                    ret = sbxpc.SBXPCDLL.ReadGLogWithPos_GPS(Program.gMachineNumber, start, end);
            }
            else
            {
                if (chkReadGlogExt.Checked)
    ...
    class SLogInfo
	{
		...
		public string manipulation
		{
			get
			{
				switch (mnpl)
				{
                    ...
                    case 43:
                        return Convert.ToString(mnpl) + "--" + "Delete QR Data";
		...
		public string finger { get { return (fpno < 10) ? Convert.ToString(fpno) : (fpno == 10) ? "Password" : (fpno == 14) ? "Face" : (fpno == 15) ? "QR" : "Card"; } }
		public string logtime { get { return string.Format("{0:D4}-{1:D2}-{2:D2} {3:D2}:{4:D2}:{5:D2}", yr, mon, day, hr, min, sec); } }
	}

	class GLogInfo
	{
        public GLogInfo(bool attend_status_as_mark)
        {
            this.attend_status_as_mark = attend_status_as_mark;
		}
		public int tmno;
        ...
        private bool attend_status_as_mark = false;
		...
		public string verify_mode
		{
			get
			{
				...
				if (attend_status_as_mark)
				{
					switch ((vmode >> 8) & 0xFF)
					{
						case 0: attend_status = "_F"; break;
						case 1: attend_status = "_S"; break;
						case 2: attend_status = "_A"; break;
						case 3: attend_status = "_I"; break;
						case 4: attend_status = "_B"; break;
						case 5: attend_status = "_P"; break;
						case 6: attend_status = "_G"; break;
						case 7: attend_status = "_V"; break;
						case 8: attend_status = "_N"; break;
					}
				}
				else
				{
				...
				switch (vm)
				{
                    ...
                    case 35: str = "FACE+FP+CARD"; break;
                    case 36: str = "QR"; break;
                    case 37: str = "FACE+QR"; break;
                    case 38: str = "FP+QR"; break;
                    case 39: str = "CARD+QR"; break;
                    case 40: str = "QR+PWD"; break;
                    case 41: str = "FACE+FP+QR"; break;
                    case 42: str = "FACE+QR+PWD"; break;
                    case 43: str = "FACE+CARD+QR"; break;
                    case 44: str = "FP+CARD+QR"; break;
                    case 45: str = "FP+QR+PWD"; break;
                }
        ...
        public string body_temperature
        {
            get
            {
                ...
                int body_temperature_10_delta = (dwGlogExt & 0xFF); // (body_temperature_10_delta: 8, mood: 2, attend_only: 1, user_expired: 1, reserved: 20)
		...
		public string mood
		{
			get
			{
				int v = ((dwGlogExt >> 8) & 0x3); // (body_temperature_10_delta: 8, mood: 2, attend_only: 1, user_expired: 1, reserved: 20)
        ...
        public string attend_only
        {
            get
            {
                int v = ((dwGlogExt >> 10) & 0x1); // (body_temperature_10_delta: 8, mood: 2, attend_only: 1, user_expired: 1, reserved: 20)
        ...
        public string expired
        {
            get
            {
                int v = ((dwGlogExt >> 11) & 0x1); // (body_temperature_10_delta: 8, mood: 2, attend_only: 1, user_expired: 1, reserved: 20)
        ...
        public string latitude
        {
            get
            {
                if (dwLatitude != 0)
                {
                    bool is_south = (dwLatitude & 0x80000000) != 0;
                    int dec = (dwLatitude & 0x7FFFFFFF) / 100000;
                    int frac = (dwLatitude & 0x7FFFFFFF) % 100000;
                    while (frac > 0 && (frac % 10) == 0)
                        frac = frac / 10;

                    string str = dec.ToString() + "." + frac.ToString();
                    if (is_south)
                        str = str + ",S";
                    else
                        str = str + ",N";
        ...
        public string longitude
        {
            get
            {
                if (dwLongitude != 0)
                {
                    bool is_west = (dwLongitude & 0x80000000) != 0;
                    int dec = (dwLongitude & 0x7FFFFFFF) / 100000;
                    int frac = (dwLongitude & 0x7FFFFFFF) % 100000;
                    while (frac > 0 && (frac % 10) == 0)
                        frac = frac / 10;
