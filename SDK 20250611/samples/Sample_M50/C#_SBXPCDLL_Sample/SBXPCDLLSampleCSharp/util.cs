...
//        Public Declare Function GetSystemDefaultLangID Lib "kernel32" () As Short
        ...
        public static int gCompressPhotoSize_32K = 32 * 1024; // 32K
        ...
        public static string ErrorPrint(int aErrorCode)
        {
            switch (aErrorCode)
            {
                ...
                case 9:
                    return "ERR_INCOMPATIBLE_TEMPLATE";
                default:
                    return "NO_ERROR";
            }
		...
		private static UInt32 ocx_calc_csum_fp(byte[] data, int size)
		{
			UInt32 csum = 0;
			for (int i = 0; i < size; i+=4)
				csum += BitConverter.ToUInt32(data, i);

			return csum;
		}

		const string FpMagic = "SmackBio";
		public static void ocx_pack_fp(byte[] dst, byte[] fp, int size)
		{
			Buffer.BlockCopy(Encoding.ASCII.GetBytes(FpMagic), 0, dst, 0, 8);
			Buffer.BlockCopy(fp, 0, dst, 8, size);
			Buffer.BlockCopy(BitConverter.GetBytes(ocx_calc_csum_fp(fp, size)), 0, dst, 8 + size, 4);
		}
	}
}
