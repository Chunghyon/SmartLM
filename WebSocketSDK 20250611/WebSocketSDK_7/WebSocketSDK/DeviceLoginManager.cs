...
namespace SmackBio.WebSocketSDK.Sample
{
    ...
    public class DeviceLoginManager : IDeviceLoginManager
    {
        ...
        {
            EvtTimeLog tl = new EvtTimeLog();
            tl.Parse(xml);
            DeviceEvent evt = new DeviceEvent { device_uid = tl.device_sn, content = xml };
            DeviceEventQueue.Enqueue(evt);

            Int64 user_id;
            string action;
            if (evt.is_userinfo_updated_event(out user_id, out action))
                DeviceUpdatedUserQueue.Enqueue(new DeviceUpdatedUserInfo { device_uid = tl.device_sn, user_id = user_id, action = action });
